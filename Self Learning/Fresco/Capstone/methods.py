    ...
    def extract_tokens(text, target):
        ...
        for t in text:
            for res in target:
                ...
                tokens = list(filter(lambda x: x.lower() not in stop_words and len(x) >=3 and x.lower()!='subject', tokens))
    ...
    def extract_features(document):
        ...
        for word in document:
            if word in SpamClassifier.uniqueWords:
                ...
                features[word] = False
        return features
trainedDataSet = nltk.classify.apply_features(extract_features,dt)
classifier = nltk.NaiveBayesClassifier.train(trainedDataSet)

testTokens = extract_tokens(test_X, [0,1])
        tresult = get_features(testTokens)
        testDataSet = nltk.classify.apply_features(extract_features,testTokens)
        accuracyPercent = nltk.classify.accuracy(classifier, testDataSet)*100
        predictedData = classifier.classify(testDataSet)
    ...
    def predict(text):
        ...
        testTokens = []
        testTokens = extract_tokens(text, [0,1])
        result = get_features(testTokens)
        ...
        for t in testDataSet:
            predictedData = classifier.classify(testTokens)
            p.append(predictedData)
